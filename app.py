...
with col2:
    ...
    st.markdown(
        ...
        <div style="
            ...
            margin-bottom:20px;
        ">
            <h1 style="margin:5px 0; line-height:1.2; color:white; font-size:32px;">
                ✨ Roblox Game Popularity Prediction 🚀
            </h1>
            <p style="margin:5px 0; font-size:18px; line-height:1.2; color:white;">
                Prediksi tingkat popularitas game di Roblox berdasarkan aktivitas pengguna menggunakan model SVM & KNN
            </p>
        </div>
        """,
        unsafe_allow_html=True
...
if svm_matrix is not None or knn_matrix is not None:
    ...
    with colA:
        if svm_matrix is not None:
            plot_matrix(svm_matrix, "Confusion Matrix - SVM", cmap_color="YlGn")
    with colB:
        if knn_matrix is not None:
            plot_matrix(knn_matrix, "Confusion Matrix - KNN", cmap_color="YlGnBu")
